...
const cashFlowAccountMonthlyChanges = `-- name: CashFlowAccountMonthlyChanges :many
SELECT
    t.account_id,
    a.currency,
...
type CashFlowAccountMonthlyChangesParams struct {
	UserID   uuid.UUID   `json:"user_id"`
	DateFrom pgtype.Date `json:"date_from"`
	DateTo   pgtype.Date `json:"date_to"`
}

type CashFlowAccountMonthlyChangesRow struct {
	AccountID uuid.UUID      `json:"account_id"`
	Currency  string         `json:"currency"`
	Month     pgtype.Date    `json:"month"`
	NetChange pgtype.Numeric `json:"net_change"`
}

func (q *Queries) CashFlowAccountMonthlyChanges(ctx context.Context, arg CashFlowAccountMonthlyChangesParams) ([]CashFlowAccountMonthlyChangesRow, error) {
	rows, err := q.db.Query(ctx, cashFlowAccountMonthlyChanges, arg.UserID, arg.DateFrom, arg.DateTo)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []CashFlowAccountMonthlyChangesRow{}
	for rows.Next() {
		var i CashFlowAccountMonthlyChangesRow
		if err := rows.Scan(
			&i.AccountID,
			&i.Currency,
			&i.Month,
...
const cashFlowAccountOpeningBalances = `-- name: CashFlowAccountOpeningBalances :many
SELECT
    a.id AS account_id,
    a.currency,
...
type CashFlowAccountOpeningBalancesParams struct {
	DateFrom pgtype.Date `json:"date_from"`
	UserID   uuid.UUID   `json:"user_id"`
}

type CashFlowAccountOpeningBalancesRow struct {
	AccountID      uuid.UUID      `json:"account_id"`
	Currency       string         `json:"currency"`
	OpeningBalance pgtype.Numeric `json:"opening_balance"`
}

func (q *Queries) CashFlowAccountOpeningBalances(ctx context.Context, arg CashFlowAccountOpeningBalancesParams) ([]CashFlowAccountOpeningBalancesRow, error) {
	rows, err := q.db.Query(ctx, cashFlowAccountOpeningBalances, arg.DateFrom, arg.UserID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []CashFlowAccountOpeningBalancesRow{}
	for rows.Next() {
		var i CashFlowAccountOpeningBalancesRow
		if err := rows.Scan(&i.AccountID, &i.Currency, &i.OpeningBalance); err != nil {
			return nil, err
		}
		items = append(items, i)
	}
...
const cashFlowCategoryMonthly = `-- name: CashFlowCategoryMonthly :many
SELECT
    t.category_id,
    t.type,
    date_trunc('month', t.date)::DATE AS month,
...
type CashFlowCategoryMonthlyParams struct {
	UserID   uuid.UUID   `json:"user_id"`
	DateFrom pgtype.Date `json:"date_from"`
	DateTo   pgtype.Date `json:"date_to"`
}

type CashFlowCategoryMonthlyRow struct {
	CategoryID pgtype.UUID    `json:"category_id"`
	Type       string         `json:"type"`
	Month      pgtype.Date    `json:"month"`
	Currency   string         `json:"currency"`
...
func (q *Queries) CashFlowCategoryMonthly(ctx context.Context, arg CashFlowCategoryMonthlyParams) ([]CashFlowCategoryMonthlyRow, error) {
	rows, err := q.db.Query(ctx, cashFlowCategoryMonthly, arg.UserID, arg.DateFrom, arg.DateTo)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	items := []CashFlowCategoryMonthlyRow{}
	for rows.Next() {
		var i CashFlowCategoryMonthlyRow
		if err := rows.Scan(
			&i.CategoryID,
			&i.Type,
			&i.Month,
...
func (q *Queries) ListTransactionYears(ctx context.Context, userID uuid.UUID) ([]int32, error) {
	rows, err := q.db.Query(ctx, listTransactionYears, userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
